...
const Multiplier = () => {
  ...
  useEffect(() => {
    const btn = document.getElementsByClassName('btn')
      btn[0].addEventListener('click', () => {
        ...
        setVal(() => { return (Number(txt) - 1) * MULTIPLIER })
      })
  ...
  return (
    <div className='w-1/4 bg-white rounded-md mx-auto p-3 shadow-md'>
      <h1 className='font-bold'>Multiplier</h1>
      <hr />
      <h3 className='text-xl font-semibold rounded-full bg-gray-100 w-min p-4 ml-auto mt-3'>{ MULTIPLIER}</h3>
